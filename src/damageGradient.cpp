...
void calDamageGradient(std::vector<mpmParticle>* particles, parametersSim param, double dx, std::map<std::string, int>* gridMap, std::vector<Grid>* nodesVec)
{
    for (int f = 0; f < particles->size(); f++) {
        struct weightAndDreri WD = calWeight(dx, (*particles)[f].position);
            ...
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    ...
                    if (weight != 0) {
                        ...
                        Eigen::Vector3d posD = (*particles)[f].position - (*nodesVec)[eid].posIndex.cast<double>() * dx;
    ...
    for (int g = 0; g < (*nodesVec).size(); g++) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    std::string ID = calculateID_string((int)(*nodesVec)[g].posIndex[0] + i - 1, (int)(*nodesVec)[g].posIndex[1] + j - 1, (int)(*nodesVec)[g].posIndex[2] + k - 1);
...
Eigen::Vector3d calDamageGradientPoint(Eigen::Vector3d pos, parametersSim param, double dx, std::map<std::string, int>* gridMap, std::vector<Grid>* nodesVec)
{
    Eigen::Vector3d deltaPoint = { 0, 0, 0 };
    Eigen::Vector3d base = (pos) / dx - Eigen::Vector3d::Constant(0.5);
    Eigen::Vector3i posIndex = base.cast<int>();

    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            for (int k = 0; k < 3; k++) {
                std::string ID = calculateID_string(posIndex[0] + i, posIndex[1] + j, posIndex[2] + k);
...
double calDamageValuePoint(Eigen::Vector3d pos, parametersSim param, double dx, std::map<std::string, int>* gridMap, std::vector<Grid>* nodesVec)
{
    double dpValue = 0;
    Eigen::Vector3d base = (pos) / dx - Eigen::Vector3d::Constant(0.5);
    Eigen::Vector3i posIndex = base.cast<int>();
