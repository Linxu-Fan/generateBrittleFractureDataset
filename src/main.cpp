...
int main()
{
	...
	else
	{
		...
		for (int k = 17; k < 700; k++)
		{
			...
			// Stpe 5: shift the simulation particles
			for (int p = 0; p < particles.size(); p++)
			{
				particles[p].position += Eigen::Vector3d::Ones();
			}


			bool generateFragments = false;
			std::tuple<bool, meshObjFormat, meshObjFormat, std::vector<meshObjFormat>> crackSurfs;
			std::vector<double> damageRatioVec; 
			std::map<int, Eigen::Vector3d> gridMapCrackNormal;
			for (int timestep = 0; timestep <= 2000 && generateFragments == false; timestep++)
			{
				...
				advanceStepMassive(param, &particles, timestep, damageRatioVec, gridMapCrackNormal, material1);
			...
			if (generateFragments == true)
			{
				...
				Eigen::Vector3d deviation = {-1.0, -1.0, -1.0};
				// output crack surface and fragments
				writeObjFile_fullName(std::get<2>(crackSurfs).vertices, std::get<2>(crackSurfs).faces, folder + "/crackSurfaceFull.obj", deviation, true);
				for (int fag = 0; fag < std::get<3>(crackSurfs).size(); fag++)
				{
					writeObjFile_fullName(std::get<3>(crackSurfs)[fag].vertices, std::get<3>(crackSurfs)[fag].faces, folder + "/fragment_"+std::to_string(fag) + ".obj", deviation, true);
				}
